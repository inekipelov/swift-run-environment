...
public enum RunEnvironment: String, CaseIterable {
    ...
    public static var current: RunEnvironment {
        BundleEnvironmentDetector(bundle: .main).runEnvironment
    }

    /// Resolves the current runtime environment using a caller-provided detector.
    ///
    /// This API is primarily useful for testing custom detection logic.
    ///
    /// - Parameter detector: A detector implementation that computes the runtime environment.
    /// - Returns: The environment reported by `detector`.
    public static func current(by detector: any RunEnvironmentDetector) -> RunEnvironment {
        detector.runEnvironment
    }
}
